...
let evenNumbers = mixedNumbers.filter(function callback(element, index, wholeArray) {
  if (element % 2 === 0) {
    return element;
  }
});
...
//Code Here
let postTaxPrices = prices.map(function callback(element, index, wholeArray) {
  return element * 1.07;
} );
...
let totalPopulation = populations.reduce(function callback(runningTotal, curElement, curIndex, wholeArray) {
...
let myStrongest = monstersInYourPocket.filter(function callback(element, index, wholeArray) {
  if (element["CP"] > 200) {
    return element;
  }
});
...
let orderTotals = orders.map(function callback(element, index, wholeArray) {
  let tax = element["tax"] + 1;
  return element["price"] * tax;
})
...
let bobsTotal = purchases.reduce(function callback(runningTotal, curElement, curIndex, wholeArray) {
  if (curElement["owner"] === "Bob") {
    return runningTotal + curElement["price"];
  }
  return runningTotal;
}, purchases[0]["owner"] === "Bob" ? purchases[0]["price"] : 0);

// let bobAlternate = purchases.filter(function(element, index, wholeArray) {
//   return element.owner === "Bob";
// }).map(function(element, index, wholeArray) {
